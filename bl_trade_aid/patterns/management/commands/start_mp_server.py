...
print(f'index in df original: {df.index}')
...
print(f'date_mark: {date_mark}')

mp = MpFunctions(data=df.copy(), freq=freq, style=mode, avglen=avglen, ticksize=ticksz, session_hr=trading_hr)
print(f'freq: {freq}, style: {mode}, avlen: {avglen}, ticksize: {ticksz}, session_hr: {trading_hr}')
...
def update_graph(n, value):
    ...
    print(f'mplist[0]:{mplist[0]}')
    distribution_hist = mplist[1]
    print(f'mplist[1]:{mplist[1]}')
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-p", "--port", type=int, default=8999, help="local port for TWS connection"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            app.run_server(
                port=options['port'], host='0.0.0.0',
                debug=True)  # debug=False if executing from ipython(vscode/Pycharm/Spyder)
